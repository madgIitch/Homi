...
import type { Flat, GenderPolicy } from '../types/room';
...
type EditFlatRouteParams = {
  flat?: Flat;
};

type LocationOption = { id: string; label: string };

export const EditFlatScreen: React.FC = () => {
  const theme = useTheme();
  const insets = useSafeAreaInsets();
  const navigation = useNavigation<StackNavigationProp<any>>();
  const route = useRoute();
  const { flat } = (route.params as EditFlatRouteParams) || {};
  ...
  useEffect(() => {
    ...
    setCity(flat.city ?? '');
    setDistrict(flat.district ?? null);
    setGenderPolicy(flat.gender_policy ?? 'mixed');
  }, [flat]);
  ...
  }, [allowedPolicies, genderPolicy]);

  useEffect(() => {
    let isMounted = true;
    const loadSelectedCity = async () => {
      if (!flat?.city_id) return;
      try {
        const cityResult = await locationService.getCityById(flat.city_id);
        if (!isMounted) return;
        if (cityResult) {
          setSelectedCity({ id: cityResult.id, label: cityResult.name });
  ...
  useEffect(() => {
    ...
    const loadSelectedPlace = async () => {
      if (!flat?.place_id) return;
      try {
        const placeResult = await locationService.getPlaceById(flat.place_id);
        if (!isMounted) return;
        if (placeResult) {
          setSelectedPlace({ id: placeResult.id, label: placeResult.name });
          setDistrict(placeResult.name);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        console.error('[EditFlat] Error cargando ciudades:', error);
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        console.error('[EditFlat] Error cargando zonas:', error);
        if (isActive) {
          setPlaces([]);
          setTopPlaces([]);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await roomService.updateFlat(flat.id, {
      ...
      try {
        ...
        console.warn('[EditFlat] Error guardando contadores:', error);
      }
      Alert.alert('Exito', 'Piso actualizado');
      navigation.goBack();
    } catch (error) {
      console.error('Error actualizando piso:', error);
      Alert.alert('Error', 'No se pudo actualizar el piso');
    } finally {
      setSaving(false);
    }
  };

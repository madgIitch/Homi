// supabase/functions/push-group-expense/index.ts
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const payload = (await req.json()) as WebhookPayload<GroupExpenseRecord>;
    ...
    (profiles ?? []).forEach((row) => {
      profileMap.set(row.id as string, row as ProfileRow);
    });
    ...
    const creatorName = buildProfileName(creator) || 'Un miembro';
    ...
    console.error('[push-group-expense] error:', message);

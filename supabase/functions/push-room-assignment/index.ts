// supabase/functions/push-room-assignment/index.ts
  ...
  switch (status) {
    case 'offered':
      return {
        ...
        body: `${actorName} te ha ofrecido ${roomTitle}.`,
      };
    case 'accepted':
      return {
        title: 'Oferta aceptada',
        body: `${actorName} acepto ${roomTitle}.`,
      };
    case 'rejected':
      return {
        title: 'Oferta rechazada',
        body: `${actorName} rechazo ${roomTitle}.`,
      };
    default:
      return {
        title: 'Actualizacion de habitacion',
        body: `${actorName} actualizo ${roomTitle}.`,
      };
  }
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const payload = (await req.json()) as WebhookPayload<AssignmentRecord>;
    ...
    const status = record.status ?? 'offered';
    ...
    for (const recipientId of recipients) {
      ...
      const dataPayload = {
        ...
        match_id: record.match_id ? String(record.match_id) : '',
        room_id: String(record.room_id),
        assignment_id: String(record.id),
    ...
    console.error('[push-room-assignment] error:', message);
